...
exports.create = async (req, res) => {
    try {
        const item = await Blog.create(normalizeMediaPayload(req.body));
...
exports.getAll = async (req, res) => {
    try {
        ...
        const items = await Blog.find(query).sort({ featured: -1, date: -1, createdAt: -1 });
...
exports.update = async (req, res) => {
    try {
        const item = await Blog.findByIdAndUpdate(req.params.id, normalizeMediaPayload(req.body), { new: true, runValidators: true });
        if (!item) return res.status(404).json({ success: false, message: 'Not found' });
