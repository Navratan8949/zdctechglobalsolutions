...
exports.create = async (req, res) => {
    try {
        const item = await CaseStudy.create(normalizeMediaPayload(req.body));
...
exports.update = async (req, res) => {
    try {
        const item = await CaseStudy.findByIdAndUpdate(req.params.id, normalizeMediaPayload(req.body), { new: true, runValidators: true });
        if (!item) return res.status(404).json({ success: false, message: 'Not found' });
