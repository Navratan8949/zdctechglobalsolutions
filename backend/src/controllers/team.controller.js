...
exports.create = async (req, res) => {
    try {
        const item = await Team.create(normalizeMediaPayload(req.body));
...
exports.getAll = async (req, res) => {
    try {
        ...
        const items = await Team.find(query).sort({ order: 1, createdAt: -1 });
...
exports.update = async (req, res) => {
    try {
        const item = await Team.findByIdAndUpdate(req.params.id, normalizeMediaPayload(req.body), { new: true, runValidators: true });
        if (!item) return res.status(404).json({ success: false, message: 'Not found' });
