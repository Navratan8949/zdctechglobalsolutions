...
type PortfolioDraft = Omit<PortfolioItem, "_id" | "createdAt" | "updatedAt">;
...
function toDraft(item: PortfolioItem): PortfolioDraft {
  return {
    slug: item.slug || "",
    title: item.title || "",
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    const loadPortfolios = async () => {
      ...
      setError(null);
      try {
        const items = unwrap<PortfolioItem[]>(await getPortfolios());
        setPortfolios(items || []);
        if (items?.length) {
  ...
  const selectPortfolio = async (id: string) => {
    ...
    setFeedback(null);
    try {
      const item = unwrap<PortfolioItem>(await getPortfolioById(id));
  ...
  const findBySlug = async () => {
    ...
    setFeedback(null);
    try {
      const item = unwrap<PortfolioItem>(
        await getPortfolioBySlug(slugLookup.trim()),
      );
      setSelectedId(item._id);
      setDraft(toDraft(item));
      setFeedback("Portfolio item loaded by slug.");
    } catch (lookupError) {
  ...
  const updateImage = (field: keyof PortfolioImage, value: string) =>
    setDraft((current) => ({
      ...current,
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setFeedback(null);
    try {
      const saved = unwrap<PortfolioItem>(
        selectedId
          ? await updatePortfolio(selectedId, draft)
  ...
  const handleDelete = async () => {
    if (
      !selectedId ||
      !window.confirm("Delete this portfolio item? This cannot be undone.")
    )
      return;
    setIsSubmitting(true);
    setError(null);
    ...
    try {
      ...
      const remaining = portfolios.filter((item) => item._id !== selectedId);
            ...
            <aside className="h-fit rounded-xl border border-white/10 bg-card/70 p-4">
              ...
              <div className="mb-4 flex gap-2">
                <Input
                  ...
                  aria-label="Find portfolio by slug"
                />
                <Button
                  type="button"
                  size="icon"
                  ...
                  aria-label="Find portfolio item"
                >
              ...
              {portfolios.length === 0 ? (
                <p className="rounded-lg border border-dashed border-white/15 p-4 text-sm leading-6 text-muted-foreground">
                  No portfolio items exist yet. Fill in the form to create the
                  first one.
                </p>
              ) : (
                <div className="space-y-2">
                  {portfolios.map((item) => (
                    <button
                      key={item._id}
                      type="button"
                      onClick={() => void selectPortfolio(item._id)}
            ...
            >
              <div className="mb-6 flex flex-col justify-between gap-3 border-b border-white/10 pb-5 sm:flex-row sm:items-start">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    All fields from the Portfolio API are available below.
                  </p>
                </div>
                {selectedId && (
                  <Button
              ...
              <div className="grid gap-5 md:grid-cols-2">
                ...
                <SelectField
                  ...
                  value={draft.status}
                  onChange={(value) =>
                    updateField("status", value as PortfolioDraft["status"])
                  }
