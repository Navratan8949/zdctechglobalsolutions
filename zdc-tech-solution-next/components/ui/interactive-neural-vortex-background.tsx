...
const InteractiveNeuralVortex = ({ children, className }: { children?: React.ReactNode, className?: string }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      (animationRef as any).current = requestAnimationFrame(render);
    };
  ...
  return (
    <div className={`relative min-h-screen w-full overflow-hidden bg-slate-900 ${className || ''}`}>
      {/* Canvas Background */}
      <canvas 
        ref={canvasRef} 
        id="neuro" 
        className="absolute inset-0 w-full h-full pointer-events-none opacity-80 z-0 mix-blend-screen"
      ></canvas>
